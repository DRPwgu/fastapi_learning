...
@router.get('/todo/by_priority/', status_code=status.HTTP_200_OK)
async def read_todo_by_priority(priority: int, db: db_dependency):
    todos = db.query(Todos).filter(Todos.priority >= priority).all()

    if todos:
        return todos
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                        detail=f'No Todos with priority greater than {priority}!')
